import { motion } from 'framer-motion';
import useWindowWidth from '../hooks/useWindowWidth';
import { useTranslation } from 'react-i18next';
import { ContentMediaSlider } from './ContentMediaSlider';
import { useContentSlot } from '../hooks/useSiteContent';

export function Hero({
  variant = 'landing',
}: {
  variant?: 'landing' | 'client-dashboard';
}) {
  const width = useWindowWidth();
  const { t, i18n } = useTranslation();
  const { media } = useContentSlot('landing_hero');

  // Admin-managed hero media replaces the built-in background only on the
  // landing hero, and only once something has been uploaded.
  const managedMedia = variant === 'landing' ? media : [];
  const hasManagedMedia = managedMedia.length > 0;

  return (
    <section
      id='home'
      className='relative h-[55vh] md:min-h-screen w-full overflow-hidden bg-cover bg-no-repeat'
      style={
        hasManagedMedia
          ? undefined
          : {
              backgroundImage:
                width < 768 ? 'url(/hero-bg-mobile.jpg)' : 'url(/hero-bg.jpg)',
              backgroundSize: width < 768 ? '170% 100%' : '100% auto',
              backgroundPosition: width < 768 ? '100% 50%' : 'center center',
            }
      }
    >
      {hasManagedMedia && (
        <ContentMediaSlider
          media={managedMedia}
          className='absolute inset-0'
          showDots={false}
        />
      )}

      <div className='relative mx-auto flex min-h-screen w-full items-start px-5 pb-12 pt-[162px] sm:px-8 sm:pt-[190px] md:px-10 md:pt-[280px] lg:mx-auto lg:pt-[250px] '>
        <div className='max-w-[240px] sm:max-w-[320px] md:max-w-none mx-auto -translate-x-[100%] md:-translate-x-[45%] flex flex-col  items-center justify-center '>
          {variant === 'landing' && (
            <>
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, ease: 'easeOut', delay: 0.2 }}
                className='-ml-30 md:pl-10'
              >
                <p
                  className={`text-[#efe0c9] font-normal font-italic text-[31px] leading-[1.08] sm:text-[46px] md:text-[62px] lg:text-[80px] ${i18n.language === 'ar' ? 'font-amiri!' : 'font-italic'}`}
                >
                  {t('hero.welcome')}
                </p>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, ease: 'easeOut', delay: 0.4 }}
                className={['en', 'fr'].includes(i18n.language) ? '' : 'mt-12'}
              >
                <h1
                  className={`text-[#efe0c9] ${i18n.language === 'en' ? 'font-italic' : 'font-bold '} text-lg md:text-[48px] leading-[0.98] tracking-[0.055em] sm:text-[68px] md:text-[76px] md:leading-[1.05] lg:text-[120px] font-thin`}
                  style={{ WebkitTextStroke: '0.3px #f8e5c6' }}
                >
                  {t('hero.al_malaki')}
                </h1>
              </motion.div>
            </>
          )}
          {variant === 'client-dashboard' && (
            <div
              className={`${i18n.language === 'ar' && 'flex flex-col-reverse'}`}
            >
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, ease: 'easeOut', delay: 0.4 }}
                className='my-5 sm:mt-6 '
              >
                <h1
                  className={`text-[#efe0c9] ${i18n.language === 'en' ? 'font-italic' : 'font-bold'} text-[48px] leading-[0.98] tracking-[0.055em] text-center sm:text-[68px] md:text-[76px] md:leading-[1.05] tracking-widest lg:text-[85px] font-thin text-center`}
                  style={{ WebkitTextStroke: '0.3px #f8e5c6' }}
                >
                  {t('clientSpace.title')}
                </h1>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, ease: 'easeOut', delay: 0.2 }}
                className=''
              >
                <p
                  className={`text-[#efe0c9] font-normal font-italic text-[31px] leading-[1.08] sm:text-[46px] md:text-[62px] lg:text-[35px] text-center  ${i18n.language === 'ar' ? 'font-amiri!' : 'font-italic'}`}
                >
                  {t('clientSpace.welcome')}
                </p>
              </motion.div>
            </div>
          )}
        </div>
      </div>
    </section>
  );
}
